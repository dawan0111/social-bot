...
class CodingMiddleware(AbstractMiddleware):
    ...
    def __init__(self, modeManager: ModeManager) -> None:
        ...
        self.settings = [
            ...
            dict(id="동물", left_command=["여우"], right_command=["사자"], up_command=["토끼"], down_command=["상어"], right_eye_command=["하마"], left_eye_command=["고래", "노래", "오래"]),
    ...
    def create_motion(self, text):
        ...
        while start_index + string_length <= len(text):
            ...
            if command in self.setting["left_command"]:
                motion.append("L")
                matched_motion = True
            elif command in self.setting["right_command"]:
                motion.append("R")
                matched_motion = True
            elif command in self.setting["up_command"]:
                motion.append("U")
                matched_motion = True
            elif command in self.setting["down_command"]:
                motion.append("D")
                matched_motion = True
            elif command in self.setting["right_eye_command"]:
                motion.append("R_E")
                matched_motion = True
            elif command in self.setting["left_eye_command"]:
                motion.append("L_E")
                matched_motion = True
